...
async function initializeApp() {
    ...
    async function initializeWorker() {
        try {
            resultDiv.textContent = 'Preparing scanner in background...';
            resultDiv.parentElement.classList.add('visible');

            worker = await Tesseract.createWorker({
                logger: message => {
                    if (message && typeof message === 'object' && message.status === 'recognizing text') {
            ...
            resultDiv.textContent = 'Scanner ready';
        } catch (error) {
    ...
    function animateNumbers() {
        ...
        if (currentIndex < sampleNumbers.length) {
            ...
            currentIndex++;
            setTimeout(() => {
                readyText.classList.remove('visible');
